...
interface MatterAccessoryClusters {
  ...
  electricalEnergyMeasurement:
    | { cumulativeEnergyImported: number }
    | { cumulativeEnergyExported: number };
}
...
export class MatterEnergyBridge {
  ...
  constructor(
    ...
    // EXPERIMENTAL SPIKE (see deviceTypeOverride import in solarAccessory.ts):
    // when supplied, this device type is used instead of the sanctioned
    // api.matter.deviceTypes.ElectricalSensor. Homebridge does not currently
    // expose a Solar-specific device type through its plugin API, even though
    // the @matter/node library it bundles internally already defines one
    // (SolarPowerDevice, Matter spec §14.3). This lets us pass that device
    // type directly, imported from our own pinned @matter/main dependency,
    // to see empirically whether it registers and pairs correctly despite
    // being a *different* module instance than the one Homebridge's own
    // Matter server uses internally. NOT intended to ship — see the spike
    // notes in solarAccessory.ts.
